...
class Build:
    ...
    def saveAudio(self, time, filename):
        return recorder.record(self, time, filename)

    def transcriber(self , audio):
        transcriber.Transcriber.transcribe(audio)
        
    def summarizer(self , transcript):
        summarizer.Summarizer.summarize(transcript)
    
    def run(self):
        for _ in range(5):
            
            print("Building audio1")
            
            ps = 1 # first pass and subsequents
            rec = threading.Thread(target=self.saveAudio, args=[60,f"sch{ps}.wav"])
            tsc = threading.Thread(target=self.transcriber, args=(f"dump.txt"))
            ps += 1
        ...
        self.summarizer(transcript="dump.txt")
